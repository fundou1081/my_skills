...
- description mentions triggers/use cases
- File size constraints (SKILL.md < 500 lines recommended)
- Required directories (optional)
"""
import sys
import re
import yaml
...
def validate_skill(skill_dir: Path) -> tuple[bool, list[str]]:
    ...
    fm_match = re.match(r"^---\n(.*?)\n---\n", content, re.DOTALL)
    if not fm_match:
        errors.append("❌ YAML frontmatter not found (must start with --- and end with ---)")
        return False, errors
    ...
    if line_count > 500:
        errors.append(f"⚠️  SKILL.md has {line_count} lines (recommended < 500, content should move to references/)")

    # 7. Forbidden files
    forbidden = ["README.md", "INSTALLATION.md", "CHANGELOG.md", "INSTALLATION_GUIDE.md", "QUICK_REFERENCE.md"]
    for f in forbidden:
        if (skill_dir / f).exists():
            errors.append(f"❌ Forbidden file found: {f} (skill should be lean, no auxiliary docs)")

    # 8. Forbidden extra fields in frontmatter
    allowed_fields = {"name", "description"}
    extra = set(fm.keys()) - allowed_fields
    if extra:
        errors.append(f"⚠️  Extra frontmatter fields (should only have name + description): {extra}")

    # 9. Resource references (check if files in references/ are mentioned in SKILL.md)
    refs_dir = skill_dir / "references"
    if refs_dir.exists():
        ref_files = list(refs_dir.glob("*.md"))
        for ref in ref_files:
            if ref.name not in content:
                errors.append(f"⚠️  references/{ref.name} not referenced in SKILL.md")
    ...
    if assets_dir.exists():
        ...
        for asset in asset_files:
            if asset.name not in content:
                errors.append(f"⚠️  assets/{asset.name} not referenced in SKILL.md")
